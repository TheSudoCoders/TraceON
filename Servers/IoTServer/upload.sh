...
DELIVERABLES="entry.py common/image_store.py common/utils.py"
...
if [[ "$?" -ne 0 ]]; then
    ...
    IAM_ROLE="$(aws iam get-role --role-name sudotrace-iot-server-role --output json | grep -oE "arn:.+role" --color=none)"
